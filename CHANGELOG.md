...
- Agents must not edit or commit on `main`; use a topic branch first.
...
- Rolls in Media: a folder linked to a roll shows a film icon and the roll's name in the folder tree, and right-clicking any folder offers "Link to roll…" (choose a roll, then the usual scan-link preview).
